...
class QuoteRepository extends BaseRepository
{
    ...
    public function saveQuote(array $input): Quote
    {
        try {
            ...
            DB::beginTransaction();
            $input['final_amount'] = $input['final_amount'];
            if ($input['final_amount'] == 'NaN') {
                $input['final_amount'] = 0;
            }
            $quoteItemInputArray = Arr::only($input, ['product_id', 'quantity', 'price', 'itemTax']);
            ...
            foreach ($quoteItemInput as $data) {
                ...
                if (auth()->user()->id == 7) {
                    foreach ($data['itemTax'] as $taxId) {
                        $tax = Tax::find($taxId);
                        $quoteItemTax = new QuoteItemTax([
                            'tax_id' => $taxId,
                            'tax' => $tax?->value ?? null 
                        ]);

                        $quoteItem->quoteItemTax()->save($quoteItemTax);
                    }
                }
