...
class QuoteController extends Controller
{
    ...
    public function store(CreateQuoteRequest $request): mixed
    {
        ...
        try {
            ...
            if( isset($attachments) && !empty($attachments)){
                $quote->attachments = json_encode( $attachments );
                $quote->save();
            }
            $quote->project_name = $request->input('project_name');
                $quote->save();
    ...
    // code by AG start
    public function quote_duplicate($quoteId){
        
        $original_quote = Quote::find($quoteId);
        $original_quote_items = $original_quote->quoteItems()->get();
        
        
        $duplicated_quote = $original_quote->replicate();
        $duplicated_quote->quote_id = Quote::generateUniqueQuoteId();
        $duplicated_quote->quote_date = date(currentDateFormat());
        $duplicated_quote->due_date = date(currentDateFormat(), strtotime("+1 day"));
        $duplicated_quote->status = 0;
        $duplicated_quote->created_at = Carbon::now();
        $duplicated_quote->save();
        
        if( !empty( $original_quote_items ) ){
            foreach( $original_quote_items as $original_quote_item ){
                $duplicated_quote_item = $original_quote_item->replicate();
                $duplicated_quote_item->quote_id = $duplicated_quote->id;
                $duplicated_quote_item->created_at = Carbon::now();
                $duplicated_quote_item->save();
                
            }
        }
        
        return redirect()->route('quotes.edit', $duplicated_quote->id);
