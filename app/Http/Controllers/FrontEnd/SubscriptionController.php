...
class SubscriptionController extends Controller
{
    ...
    {
        $current_plan = PlanVendor::where('vendor_id', $arrData['vendor_id'])->where('status', 1)->first();
        $plan = MembershipPlan::find($arrData['plan_id']);
        if($current_plan){
        $current_plan_data = MembershipPlan::find($current_plan->plan_id);
            
            if($current_plan_data->level > $plan->level){
                $mailData['subject'] = "Plan Downgrade Successfully.";
                $mailData['body'] = 'Plan Downgrade Successfully.';
                $mailData['recipient'] = $arrData['email'];
                BasicMailer::sendMail($mailData);
            }
            else if($current_plan_data->level < $plan->level){
                $mailData['subject'] = "Plan Upgrade Successfully.";
                $mailData['body'] = 'Plan Upgrade Successfully';
                $mailData['recipient'] = $arrData['email'];
                BasicMailer::sendMail($mailData);
            }
        }
        else{
            $mailData['subject'] = 'New Plan Purchase Successfully.';
        ...
        if(isset($arrData['complete_payment']))
        {
            ...
            $plan_vendor->payment_method = $arrData['payment_method'];
            $plan_vendor->total = $arrData['total'];
            $plan_vendor->name = $arrData['name'];
            $plan_vendor->email = $arrData['email'];
            $plan_vendor->contact = $arrData['contact'] ?? '';
