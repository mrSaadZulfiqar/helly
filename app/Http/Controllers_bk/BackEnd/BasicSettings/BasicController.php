...
class BasicController extends Controller
{
  ...
  public function contact_page()
  {
    $data = DB::table('basic_settings')
      ->select('email_address', 'contact_number', 'address')
      ->first();

    return view('backend.basic-settings.contact', ['data' => $data]);
  }
  ...
  public function equipmentTaxAmount()
  {
    $data['tax'] = DB::table('basic_settings')->select('equipment_tax_amount')->first();

    $data['commission'] = Commission::first();

    return view('backend.instrument.tax', $data);
  }

  public function updateEquipmentTaxAmount(Request $request)
  {
    $rules = [
      'equipment_tax_amount' => 'required|numeric'
    ];
    ...
    DB::table('basic_settings')->updateOrInsert(
      ['uniqid' => 12345],
      ['equipment_tax_amount' => $request->equipment_tax_amount]
    );

    $commission = Commission::first();
    $commission->equipment_commission = $request->equipment_commission;
    $commission->save();
  ...
  public function update_general_setting(Request $request)
  {
    ...
    DB::table('basic_settings')->updateOrInsert(
      ...
      [
        ...
        'base_currency_text_position' => $request->base_currency_text_position,
        'base_currency_rate' => $request->base_currency_rate,
        'subscription_enable' => $request->subscription_enable,
      ]
    );
