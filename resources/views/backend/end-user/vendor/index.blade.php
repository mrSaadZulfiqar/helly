                          ...
                          <td>
                            <div class="dropdown">
                              <button class="btn btn-secondary dropdown-toggle btn-sm" type="button"
                                id="dropdownSmsButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <i class="fa fa-comment"></i>
                              </button>

                              <div class="dropdown-menu" aria-labelledby="dropdownSmsButton">
                                <a class="dropdown-item ag-send-sms-btn" href="{{route('admin.vendor_management.vendor.chat', $vendor->id)}}?phone_={{$vendor->phone}}" data-phone_="{{$vendor->phone}}">{{$vendor->phone}}</a>
                                <?php 
                                    $additional_contact = App\Models\AdditionalContact::where('vendor_id',$vendor->id)->get()->toArray();;
                                ?>
                                <?php if( !empty( $additional_contact ) ){ ?>
                                    <?php foreach($additional_contact as $contact){ ?>
                                        <?php if($contact['phone_full'] != ''){ ?>
                                            <a class="dropdown-item ag-send-sms-btn" href="{{route('admin.vendor_management.vendor.chat', $vendor->id)}}?phone_={{$contact['phone_full']}}" data-phone_="{{$contact['phone_full']}}">{{$contact['phone_full']}}</a>
                                        <?php } ?>
                          ...
                          <td>
                            <div class="dropdown">
                              ...
                              <div class="dropdown-menu" aria-labelledby="dropdownCallButton">
                                <a class="dropdown-item ag-send-sms-btn" href="{{route('admin.vendor_management.vendor.calling', $vendor->id)}}?phone_={{$vendor->phone}}" data-phone_="{{$vendor->phone}}">{{$vendor->phone}}</a>
                                <?php 
                                    $additional_contact = App\Models\AdditionalContact::where('vendor_id',$vendor->id)->get()->toArray();;
                                ?>
                                <?php if( !empty( $additional_contact ) ){ ?>
                                    <?php foreach($additional_contact as $contact){ ?>
                                        <?php if($contact['phone_full'] != ''){ ?>
                                            <a class="dropdown-item ag-send-sms-btn" href="{{route('admin.vendor_management.vendor.calling', $vendor->id)}}?phone_={{$contact['phone_full']}}" data-phone_="{{$contact['phone_full']}}">{{$contact['phone_full']}}</a>
                                        <?php } ?>
