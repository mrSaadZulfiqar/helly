        ...
        <div class="card-body">
          <div class="row">
            <div class="col-lg-8 offset-lg-2">
              ...
              <form action="{{ route('vendor.equipment_booking.update', [ 'id' => $details->id ]) }}" method="POST"  enctype="multipart/form-data" >
                @csrf
                <div class="row">
                              ...
                              <label>{{ __('Select Equipment*') }}</label>
                               <select name="equipment_id" id="equipment_id" class="form-control select2" required>
                                 ...
                                 @foreach($equipments as $equipment)
                                    <option value="{{ $equipment->id }}"  @if($details->equipment_id == $equipment->id) selected @endif title="{{ $equipment->content[0]->equipment_category_id }}">{{ $equipment->content[0]->title }}</option>
                                  @endforeach
                              </select>
                  ...
                  <div class="col-lg-6">
                    ...
                    @endphp
                        ...
                        <input type="text" id="date-range-eq"  placeholder="{{ __('Select Booking Date') }}"
                               name="dates" value="{{ $date_value }}" class="form-control" required>
                  ...
                  <div class="col-lg-6">
                    @php
                        $equipment_category = \App\Models\Instrument\EquipmentContent::where('equipment_category_id', $details->equipment_id)->first();
                    @endphp
                            <div class="form-group @if($equipment_category != null) @if($equipment_category->equipment_category_id != 90) {{'d-none'}} @endif @else {{'d-none'}} @endif">
...
$(document).on('change', '#extra_services, #type_of_rental, #live_load, #is_emergency, #location_field', function(){
   ...
   $('#select2_cus').select2();
    ...
    $('#equipment_id').on("change", function(){
        ...
        if(eqCatId != 90){
            $('#customer_punchoutlist').parent().removeClass('d-block');
            $('#customer_punchoutlist').parent().addClass('d-none');
        }else{
            $('#customer_punchoutlist').parent().removeClass('d-none');
            $('#customer_punchoutlist').parent().addClass('d-block');
        }
    })
    </script>
  @endsection
